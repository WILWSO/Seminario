...
DROP POLICY IF EXISTS "Anyone can read lesson contents of active courses" ON lesson_contents;
DROP POLICY IF EXISTS "Teachers can manage lesson contents of own courses" ON lesson_contents;
...
DROP TRIGGER IF EXISTS update_lesson_contents_updated_at ON lesson_contents;
CREATE TRIGGER update_lesson_contents_updated_at 
  BEFORE UPDATE ON lesson_contents
  FOR EACH ROW 
  EXECUTE FUNCTION update_updated_at_column();

-- Migrar dados existentes das lições apenas se a tabela estiver vazia
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM lesson_contents LIMIT 1) THEN
